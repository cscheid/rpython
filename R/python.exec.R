...
python.exec.string <- function(python.code)
{
  result <- .C("py_exec_code", paste(python.code, collapse = "\n"), exit.status = integer(1), PACKAGE = "rPython")$exit.status
  return(result)
}
...
python.exec <- function(python.code)
{
  ...
  cmd <- transform(c, envir=parent.frame())
